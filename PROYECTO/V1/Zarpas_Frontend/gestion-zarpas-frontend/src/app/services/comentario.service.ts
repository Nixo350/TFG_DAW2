...
import { Comentario, ComentarioRequest } from '../modelos/Comentario'; // Importa Comentario y ComentarioRequest

@Injectable({
  providedIn: 'root'
})
export class ComentarioService {
  private baseUrl = 'http://localhost:9000/api/comentarios';
  ...
  actualizarComentario(idComentario: number, texto: string): Observable<Comentario> {
    const requestBody = { texto: texto }; // Solo necesitamos enviar el texto
  ...
  eliminarComentario(idComentario: number): Observable<void> {
    return this.http.delete<void>(`${this.baseUrl}/${idComentario}`, { headers: this.getAuthHeaders() });
  }
}